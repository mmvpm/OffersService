...
Сервис объявлений по типу Авито, где пользователи смогут размещать/редактировать/удалять объявления, искать товары в поиске, ставить продавцам оценки 
...
Наполнение сервиса контентом (товарами) будет осуществляться через парсинг объявлений с авито
...
Всё как и задумывалось, кроме пары пунктов:
- нет полноценного поиска по объявлениям (не успел добраться до него)
- парсер будет брать объявления не с Авито, а я Юлы, так как это вышло чуть дешевле — на Юле менее активно борются с парсерами
- для хранения сессии вместо sql или in-memory использовал Redis (так гораздо интереснее)
