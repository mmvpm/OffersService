...
lazy val nemia = (project in file("nemia"))
  .dependsOn(common)
  .settings(
    name := "nemia",
...
lazy val parseidon = (project in file("parseidon"))
  .dependsOn(common)
  .settings(
    name := "parseidon",
    libraryDependencies ++= Seq(
...
lazy val `nemia-int-test` = (project in file("nemia-int-test"))
  .dependsOn(nemia % "provided -> provided;compile -> compile;test -> test; runtime -> runtime")
  .settings(
    publish / skip := true,
    libraryDependencies ++= Seq(
      sttpClient,
...
lazy val root = (project in file("."))
  ...
  .aggregate(common, nemia, parseidon)
